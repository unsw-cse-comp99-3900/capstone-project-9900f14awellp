...
export default function EnhancedTable() {
  ...
  const handleEdit = (selectedIds) => {
    ...
    axios
      .get(`http://localhost:8000/invoice/invoice-draft?id=${id}`, {
        headers: {
      ...
      .then((response) => {
        ...
        updateInvoiceData({ editBefore: true });
        updateInvoiceData({ draftId: response.data.data.id });
        goGUI();
      })
      .catch((error) => {
  ...
  const handleDelete = (selectedIds) => {
    axios
      .delete("http://localhost:8000/invoice/invoice-draft", {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const fetchDraftData = useCallback(() => {
    axios
      .get("http://localhost:8000/invoice/invoice-draft", {
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
        },
      ...
      .then((response) => {
        ...
        const dData = response.data.data.map((item) =>
          createData(
            item.id,
            item.invoice_num,
            item.prograss,
